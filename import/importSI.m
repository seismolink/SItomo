function [sel_data] = importSI(sel_data)
% control function to gather nescessary parameters and invoke the
% import of SI data from SplitRacer or reading from a list
%
...
if ~exist([sel_data.SI_dir '/input/'],'dir')
    mkdir([sel_data.SI_dir '/input/']);
end
if exist([sel_data.SI_dir '/input/SIdata.mat'],'file')
    load([sel_data.SI_dir '/input/SIdata.mat']);
else
srflag = input('Import from SplitRacer? 1=yes/0=no :  ');
if srflag
    if ~input_flag
        finflag = 0;
        n = 1;
    ...
    if ~input_flag
        sel_data.reevaluate = input('Re-evaluate (1) or read pre-calculated splitting intensities (0)?');
    end

    RES = import_SR(sel_data);
    save([sel_data.SI_dir '/input/SIdata.mat'],'RES')
else
    sel_data.SI_file = input('Provide absolute path to SI file: ','s');
    RES = readSIlist(sel_data);
    save([sel_data.SI_dir '/input/SIdata.mat'],'RES')
end
end
