function sets = getsampling(data,maxfrac,vflag,dec,zlim,nx,ny,nz,edgeadd,nny,showflag,printflag,resultsfolder,modelname)
...
w = zeros(size(data.x));
for i = 1:nx
    idx = ind(X_ST>=x(i)&X_ST<=x(i+1));
    w(idx) = 1./length(idx);
end
% compute sensitivity
[~,EK_X,~,~,EK_X2,~,~,~]=SIsensitivity(data,1:length(data.x),w,1,vflag,dec,0.1.*ones(nx,ny,nz),pi/4.*ones(nx,ny,nz),zeros(nx,ny,nz),zlim.*1000,nx,ny,nz,edgeadd,nny);
...
if printflag
    ...
    ylabel('Depth in [km]')
    print(fig,[resultsfolder '/' modelname '_sensitivity.jpg'],'-djpeg','-r300')
    if ~showflag
        close(fig);
    end
    if showflag
    ...
    xlabel('x Profile in [km]')
    ylabel('Depth in [km]')
    print(fig,[resultsfolder '/' modelname '_nn.jpg'],'-djpeg','-r300')
    if ~showflag
        close(fig)
    end
end
...
for i = 1:nx
    for j = 1:ny
        for k = 1:nz
            ...
            sets(i,j,k).ar = length(ioi).*(xx(2)-xx(1)).*(zz(2)-zz(1));
            rr(i,j,k) = dd(ix);
...
if printflag
    ...
    xlabel('x Profile in [km]')
    ylabel('Depth in [km]')
    print(fig,[resultsfolder '/' modelname '_cells.jpg'],'-djpeg','-r300')
    if ~showflag
        close(fig);
    end
end
