function [model,result] = DoInvert2Dbfgs(data,thetaflag,vflag,wflag,zlim,nx,ny,nz,ix,iz,edgeadd,nny,sets,N1,N2,n1,n2,nevmax,initflag,strength_in,phi_in,theta_in,alpha,dec,showflag,printflag,graphicsfolder,filename,addname)
...
while kk<N1
    ...
    if printflag
        if showflag
    %         fig = figure('Position',[80 80 620 500]);
    ...
    indoi = randperm(length(data.x),noi);
        ...
        problem.objective = @(x)SIsensitivityBFGS(x,funinp,fig);
...
%                     C_0 = reshape(c0(k)',nx,ny,nz);
                    A_0 = reshape(a0',ix,iz);
                    B_0 = reshape(b0',ix,iz);
                    C_0 = reshape(c0',ix,iz);
                else
%                     A_0 = reshape(mm(k)',nx,ny,nz);
%                     B_0 = reshape(mm(k+max(k(:)))',nx,ny,nz);
%                     C_0 = reshape(mm(k+2.*max(k(:)))',nx,ny,nz);
                    A_0 = reshape(mm(1:ix*iz)',ix,iz);
                    B_0 = reshape(mm(ix*iz+1:2*ix*iz)',ix,iz);
                    C_0 = reshape(mm(2*ix*iz+1:3*ix*iz)',ix,iz);
                end
            else
                if initflag
                    a0 = zeros(1,nx*nz);
...
%                 end
%                 A_0 = reshape(A0n,ix,iz);
%                 B_0 = reshape(B0n,ix,iz);
%                 C_0 = reshape(C0n,ix,iz);
%             end
            if initflag
                a0 = A_0(:);
                b0 = B_0(:);
