function [dWF]=dsiforward(data,indoi,vflag,strength_in,phi_in,theta_in,poit,zlim,nx,ny,nz,edgeadd,nny)
...
y_1=Y_ST(iiii);
z_1=0;
[vi] = ForwardSIvi(per(iiii),par(iiii),AZIMUTH(iiii),x_1,y_1,z_1,Xp,Yp,Zp,omega,alpha_p,beta_p,strength_p,phi_p,theta_p,vflag);
for i = 1:sum(poit)
    dWF(iiii).vi(i) = sum(vi(poi2==i)).*dx.*dy.*dz;
end
end
