function [dSKxTr] = ForwardSIsensitivityX(per,par,AZIMUTH,x_1,y_1,z_1,X,Y,Z,omega,alpha_0,beta_0,strength_in,phi_in,theta_in,vflag)
...
dSKxTr = (eps.*(eNFTxy.*gNF+eMFTxy.*gMF+eFFTxy.*gFF)+del.*(dNFTxy.*gNF+dMFTxy.*gMF+dFFTxy.*gFF)+gam.*(gNFTxy.*gNF+gMFTxy.*gMF+gFFTxy.*gFF));%.*R.^0.5;
