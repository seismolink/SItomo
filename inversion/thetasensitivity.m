function [EKthetan]=thetasensitivity(data,WF,indoi,vflag,strength_in,phi_in,theta_in,poit,zlim,nx,ny,nz,edgeadd,nny)
...
if abs(misfit)>0.01
[dSKthetaTr] = ForwardSIsensitivityTheta(per(iiii),par(iiii),AZIMUTH(iiii),x_1,y_1,z_1,Xp,Yp,Zp,omega,alpha_p,beta_p,strength_p,phi_p,theta_p,vflag);
SKtheta = sum(real(dSKthetaTr(:))).*dx.*dy.*dz.*misfit./sum(poi);
end

temp2 = SKtheta;
EKtheta = EKtheta+temp2;
end
EKthetan = EKtheta;
