function [vi] = ForwardSIvi(per,par,AZIMUTH,x_1,y_1,z_1,X,Y,Z,omega,alpha_0,beta_0,strength_in,phi_in,theta_in,vflag)
...
vi = (eps.*(eNFTxy.*gNF+eMFTxy.*gMF+eFFTxy.*gFF)+del.*(dNFTxy.*gNF+dMFTxy.*gMF+dFFTxy.*gFF)+gam.*(gNFTxy.*gNF+gMFTxy.*gMF+gFFTxy.*gFF)).*strength_in;%.*R.^0.5;
