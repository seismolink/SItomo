function [model,result] = DoInvert2Dmcmc(data,thetaflag,vflag,zlim,nx,ny,nz,edgeadd,nny,xstepini,phistepini,thetastepini,N1,N2,n1,n2,nevmax,initflag,strength_in,phi_in,theta_in,smoothpar,showflag,printflag,graphicsfolder,filename,addname)
...
noc = N1;
ifpmts = 1:length(X(:));
xini = 0.01.*ones(noc,1);
phiini = rand.*pi.*ones(noc,1);
...
[WFref,~]=SIforward(data,1:length(data.si),0,vflag,A_0,B_0,C_0,zlim.*1000,nx,ny,nz,edgeadd,nny);
...
while roi<0.01%eps
    ...
    L2 = likelyhood_smooth(data,1:length(data.si),zeros(size(data.si)),sig,A_0,B_0,C_0,smoothpar);
    roi = L2./L1;
end
...
% Recalculate reference
    if ~mod(nmc,n1)
        ...
        indoio = indoi;
        indoi = randperm(length(data.x),noi);
        [WFrefn,~]=SIforward(data,1:length(data.si),0,vflag,A_0,B_0,C_0,zlim.*1000,nx,ny,nz,edgeadd,nny);
        dref = 0;
    ...
    if ~mod(nmc,2)
        ...
        [EKphi]=phisensitivity(data,WFref(indoi),indoi,vflag,A_0,B_0,C_0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
        ...
        [EKx]=xsensitivity(data,WFref(indoi),indoi,vflag,A_0,B_0,C_0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
        ...
        [EKtheta]=thetasensitivity(data,WFref(indoi),indoi,vflag,A_0,B_0,C_0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
        ...
        [dWF]=dsiforward(data,indoi,vflag,A0,B0,C0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
        for i = 1:length(dWF)
        ...
        if bdmflag == 1
            ...
            % calculate gradient for phi
            [EKphi]=phisensitivity(data,WFref(indoi),indoi,vflag,A0,B0,C0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
            ...
            EKphi = EKphi+mean(Rp(Idx==ioi));
            % calculate gradient for x
            [EKx]=xsensitivity(data,WFref(indoi),indoi,vflag,A0,B0,C0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
            sig1 = A0.*cos(B0).*cos(C0);
            sig2 = A0.*sin(B0).*cos(C0);
            sig3 = A0.*sin(C0);
            L1 = zeros(size(sig1));
            ...
            if thetaflag && thetaflag2
                % calculate gradient for theta
                [EKtheta]=thetasensitivity(data,WFref(indoi),indoi,vflag,A0,B0,C0,Idx==ioi,zlim.*1000,nx,ny,nz,edgeadd,nny);
                sig1 = A0.*cos(B0).*cos(C0);
                sig2 = A0.*sin(B0).*cos(C0);
                sig3 = A0.*sin(C0);
                L1 = zeros(size(sig1));
            ...
            if isempty(id2)
                ...
                [dWF]=dsiforward(data,indoi,vflag,A0,B0,C0,id2,zlim.*1000,nx,ny,nz,edgeadd,nny);
                for i = 1:length(dWF)
        ...
        if bdmflag == 3
            ...
            while sum(id) == 0
                ...
                dd = sqrt((X(ifpmts)-x1).^2+(Z(ifpmts)-z1).^2);
                [~,ii] = min(dd);
                temp2 = ifpmts(ii);
                newvn = vn;
            ...
            C0 = reshape(C0',nx,ny,nz);
            % calculate si correction for new model
            [dWF]=dsiforward(data,indoi,vflag,A0,B0,C0,id,zlim.*1000,nx,ny,nz,edgeadd,nny);
    ...
    if showflag || printflag
        ...
        drawnow;
        apl = squeeze(A_0)'.*100;
        bpl = squeeze(B_0)'./pi.*180;
        cpl = squeeze(C_0)'./pi.*180;
        set(fig, 'CurrentAxes', splt2);
        scatter(X(:),Z(:),40,apl(:),'filled');
