function [model,result] = DoInvert2Dgd(data,thetaflag,vflag,wflag,zlim,nx,ny,nz,ix,iz,edgeadd,nny,sets,xstepini,phistepini,thetastepini,N1,N2,n1,n2,nevmax,initflag,strength_in0,phi_in0,theta_in0,smoothpar,dec,showflag,printflag,graphicsfolder,filename,addname)
...
if wflag
    ...
    for i = 1:nx
        stats(i).ind = ind(X_ST>=x(i)&X_ST<=x(i+1));
...
for kk = 1:N1
    ...
    while (xstep > 0.0075|| phistep > 0.02 || thetastep > 0.02) && ii < N2
        ...
        [WF,EK_X,EK_phi,EK_theta,~,~,~,timestep]=SIsensitivity(data,indoi,w,0,vflag,dec,strength_in,phi_in,theta_in,zlim.*1000,nx,ny,nz,edgeadd,nny);
        ...
        if showflag || printflag
            subplot(4,1,1)
            scatter(xpl0,bpl,cpl*30,sipl0,'filled');
            hold on;
            ...
            hold off
            subplot(4,1,2)
            imagesc(x,z,mod(squeeze(phi_in(:,1,:).*180./pi),180)')
            ...
            subplot(4,1,3)
            imagesc(x,z,squeeze(strength_in(:,1,:)*100)')
            axis([min(xpl)-5*(xx(2)-xx(1))/10 max(xpl)+5*(xx(2)-xx(1))/10 min(z) max(z)])
            ylabel('z in [km]','FontSize',9)
            caxis([0 100]);
            caxis([-1 25])
            ...
            subplot(4,1,4)
            imagesc(x,z,squeeze(theta_in(:,1,:)*180/pi)')
            axis([min(xpl)-5*(xx(2)-xx(1))/10 max(xpl)+5*(xx(2)-xx(1))/10 min(z) max(z)])
            ylabel('z in [km]','FontSize',9)
        ...
        if aflag
            ...
            EKx = transform_kernel2D(EK_X,sets);
            ...
            while misfit >= misfito && nstep < 5
                ...
                [WF,timestep]=SIforward(data,indoi,0,vflag,strength_in,phi_in,theta_in,zlim.*1000,nx,ny,nz,edgeadd,nny);
                if ~showflag
                    fid = fopen([workdir '/results/log' addname '.txt'],'at');
                    fprintf(fid,'Done in %f seconds\n',timestep);
                    fclose(fid);
            ...
            EKphi = transform_kernel2D(EK_phi,sets);
        ...
        if bflag
            ...
            EKtheta = transform_kernel2D(EK_theta,sets);
