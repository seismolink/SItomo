function [EKxn]=xsensitivity(data,WF,indoi,vflag,strength_in,phi_in,theta_in,poit,zlim,nx,ny,nz,edgeadd,nny)
...
if abs(misfit)>0.01
[dSKxTr] = ForwardSIsensitivityX(per(iiii),par(iiii),AZIMUTH(iiii),x_1,y_1,z_1,Xp,Yp,Zp,omega,alpha_p,beta_p,strength_p,phi_p,theta_p,vflag);
SKx = sum(real(dSKxTr(:))).*dx.*dy.*dz.*misfit./sum(poi);
end
